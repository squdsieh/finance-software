...
import { Plus, Search, Download, Send, Eye, Copy, XCircle, CreditCard, Trash2, MoreHorizontal } from 'lucide-react';
...
export function InvoicesPage() {
  ...
  const fetchInvoices = async (page = 1) => {
    setLoading(true);
    try {
      const res = await invoicesApi.list({ page, limit: 25, search, status: statusFilter !== 'all' ? statusFilter : undefined });
  ...
  const handleDuplicate = async (id: string) => {
    try {
      await invoicesApi.duplicate(id);
  ...
  const columns = [
    {
      key: 'invoice_number',
      header: 'Invoice #',
      render: (inv: any) => <span className="font-medium text-primary-600">{inv.invoice_number}</span>,
    },
    { key: 'customer_name', header: 'Customer' },
    {
      key: 'invoice_date',
      header: 'Date',
      render: (inv: any) => new Date(inv.invoice_date).toLocaleDateString(),
    },
    {
      key: 'due_date',
      header: 'Due Date',
      render: (inv: any) => {
        ...
        const isOverdue = inv.status !== 'paid' && inv.status !== 'void' && due < new Date();
        return (
    ...
    },
    {
      key: 'total_amount',
      header: 'Amount',
      render: (inv: any) => `AED ${parseFloat(inv.total_amount || 0).toFixed(2)}`,
    },
    {
      key: 'balance_due',
      header: 'Balance Due',
      render: (inv: any) => {
        ...
        return (
          <span className={balance > 0 ? 'font-medium text-gray-900 dark:text-white' : 'text-gray-500'}>
        ...
        <div className="relative flex items-center gap-1">
          ...
          <div className="relative">
            <button
              onClick={(e) => {
                e.stopPropagation();
                setShowActions(showActions === inv.id ? null : inv.id);
              }}
              className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"
            >
            ...
            {showActions === inv.id && (
              <div className="absolute right-0 top-8 z-10 w-40 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1">
                ...
                {inv.status !== 'void' && (
                  <button
                    onClick={(e) => { e.stopPropagation(); handleVoid(inv.id); }}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Create, send, and track your invoices</p>
        </div>
      ...
      <Card padding={false}>
        ...
        <Table columns={columns} data={invoices} loading={loading} emptyMessage="No invoices yet. Create your first invoice!" />
      ...
      <Modal
        ...
        title={`Record Payment - ${showPayment?.invoice_number || ''}`}
        footer={
          <>
